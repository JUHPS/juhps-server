...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    JUJIMEIZUO_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
    ...
    if (cb) {
        ...
        JUJIMEIZUO_ASSERT2(event_ctx.fiber->getState() == Fiber::RUNNING, "state=" << event_ctx.fiber->getState());
    }
    return 0;
}
...
bool IOManager::cancelAll(int fd) {
    ...
    JUJIMEIZUO_ASSERT(fd_ctx->events == 0);
    return true;
}
...
void IOManager::tickle() {
    JUJIMEIZUO_LOG_DEBUG(g_logger) << "tickle";
    if(!hasIdleThreads()) {
        return;
    }
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        if( JUJIMEIZUO_UNLIKELY(stopping(next_timeout))) {
            JUJIMEIZUO_LOG_DEBUG(g_logger) << "name=" << getName() << "idle stopping exit";
            break;
        }
