#ifndef __JUJIMEIZUO_LOG_H__
#define __JUJIMEIZUO_LOG_H__
...
#define JUJIMEIZUO_LOG_LEVEL(logger , level) \
    if(level <= logger->getLevel()) \
        jujimeizuo::LogEventWrap(logger, jujimeizuo::LogEvent::ptr(new jujimeizuo::LogEvent(logger->getName(), \
            level, __FILE__, __LINE__, jujimeizuo::GetElapsedMS() - logger->getCreateTime(), \
            jujimeizuo::GetThreadId(), jujimeizuo::GetFiberId(), time(0), jujimeizuo::GetThreadName()))).getLogEvent()->getSS()

#define JUJIMEIZUO_LOG_FATAL(logger) JUJIMEIZUO_LOG_LEVEL(logger, jujimeizuo::LogLevel::FATAL)

#define JUJIMEIZUO_LOG_ALERT(logger) JUJIMEIZUO_LOG_LEVEL(logger, jujimeizuo::LogLevel::ALERT)

#define JUJIMEIZUO_LOG_CRIT(logger) JUJIMEIZUO_LOG_LEVEL(logger, jujimeizuo::LogLevel::CRIT)
...
#define JUJIMEIZUO_LOG_NOTICE(logger) JUJIMEIZUO_LOG_LEVEL(logger, jujimeizuo::LogLevel::NOTICE)
...
#define JUJIMEIZUO_LOG_FMT_LEVEL(logger, level, fmt, ...) \
    if(level <= logger->getLevel()) \
        jujimeizuo::LogEventWrap(logger, jujimeizuo::LogEvent::ptr(new jujimeizuo::LogEvent(logger->getName(), \
            level, __FILE__, __LINE__, jujimeizuo::GetElapsedMS() - logger->getCreateTime(), \
            jujimeizuo::GetThreadId(), jujimeizuo::GetFiberId(), time(0), jujimeizuo::GetThreadName()))).getLogEvent()->printf(fmt, __VA_ARGS__)

#define JUJIMEIZUO_LOG_FMT_FATAL(logger, fmt, ...) JUJIMEIZUO_LOG_FMT_LEVEL(logger, jujimeizuo::LogLevel::FATAL, fmt, __VA_ARGS__)

#define JUJIMEIZUO_LOG_FMT_ALERT(logger, fmt, ...) JUJIMEIZUO_LOG_FMT_LEVEL(logger, jujimeizuo::LogLevel::ALERT, fmt, __VA_ARGS__)

#define JUJIMEIZUO_LOG_FMT_CRIT(logger, fmt, ...) JUJIMEIZUO_LOG_FMT_LEVEL(logger, jujimeizuo::LogLevel::CRIT, fmt, __VA_ARGS__)
...
#define JUJIMEIZUO_LOG_FMT_NOTICE(logger, fmt, ...) JUJIMEIZUO_LOG_FMT_LEVEL(logger, jujimeizuo::LogLevel::NOTICE, fmt, __VA_ARGS__)
...
public:
    ...
    /**
     * @brief 构造函数
     * @param[in] pattern 格式模板，参考jujimeizuo与log4cpp
     * @details 模板参数说明：
     * - %%m 消息
     * - %%p 日志级别
     * - %%c 日志器名称
