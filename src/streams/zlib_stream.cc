...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    JUJIMEIZUO_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    JUJIMEIZUO_ASSERT((window_bits >= 8 && window_bits <= 15));
    JUJIMEIZUO_ASSERT((memlevel >= 1 && memlevel <= 9));
...
jujimeizuo::ByteArray::ptr ZlibStream::getByteArray() {
