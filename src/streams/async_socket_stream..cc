...
void AsyncSocketStream::Ctx::doRsp() {
    ...
    if(!jujimeizuo::Atomic::compareAndSwapBool(scheduler, scd, (Scheduler*)nullptr)) {
        return;
    }
    if(!scd || !fiber) {
        return;
...
bool AsyncSocketStream::start() {
    if(!m_iomanager) {
        m_iomanager = jujimeizuo::IOManager::GetThis();
    }
    if(!m_worker) {
        m_worker = jujimeizuo::IOManager::GetThis();
    }
...
void AsyncSocketStream::doRead() {
    ...
    JUJIMEIZUO_LOG_DEBUG(g_logger) << "doRead out " << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite() {
    ...
    JUJIMEIZUO_LOG_DEBUG(g_logger) << "doWrite out " << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
bool AsyncSocketStream::innerClose() {
    JUJIMEIZUO_ASSERT(m_iomanager == jujimeizuo::IOManager::GetThis());
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get() {
    ...
    for(uint32_t i = 0; i < m_size; ++i) {
        auto idx = jujimeizuo::Atomic::addFetch(m_idx, 1);
