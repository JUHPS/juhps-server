...
Fiber::Fiber() {
    ...
    if (getcontext(&m_ctx)) {
        JUJIMEIZUO_ASSERT2(false, "getcontext");
    }

    ++s_fiber_count;
    m_id = s_fiber_id++; // 协程id从0开始，用完加1

    JUJIMEIZUO_LOG_DEBUG(g_logger) << "Fiber::Fiber() main id = " << m_id;
}
...
Fiber::~Fiber() {
    ...
    if (m_stack) {
        ...
        JUJIMEIZUO_LOG_DEBUG(g_logger) << "dealloc stack, id = " << m_id;
    } else {
        // 没有栈，说明是线程的主协程
        JUJIMEIZUO_ASSERT(!m_cb);              // 主协程没有cb
        JUJIMEIZUO_ASSERT(m_state == RUNNING); // 主协程一定是执行状态
...
void Fiber::resume() {
    JUJIMEIZUO_ASSERT(m_state != TERM && m_state != RUNNING);
    SetThis(this);
    m_state = RUNNING;
    ...
    if (m_runInScheduler) {
        if (swapcontext(&(Scheduler::GetMainFiber()->m_ctx), &m_ctx)) {
            JUJIMEIZUO_ASSERT2(false, "swapcontext");
        }
    } else {
...
void Fiber::yield() {
    ...
    JUJIMEIZUO_ASSERT(m_state == RUNNING || m_state == TERM);
