    ...
    do {                                                                                                   \
        ...
        if (0) {                                                                                           \
            std::cout << "<key>:" << str.substr(last, key - last)                                          \
                      << " <decoded>:" << jujimeizuo::StringUtil::UrlDecode(str.substr(last, key - last))       \
                      << " <value>:" << str.substr(key + 1, pos - key - 1)                                 \
                      << " <decoded>:" << jujimeizuo::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)) \
                      << std::endl;                                                                        \
        }                                                                                                  \
                                                                                                           \
        m.insert(std::make_pair(jujimeizuo::StringUtil::UrlDecode(trim(str.substr(last, key - last))),          \
                                jujimeizuo::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));        \
        if (pos == std::string::npos) {                                                                    \
            break;                                                                                         \
        }                                                                                                  \
        ++pos;                                                                                             \
...
void HttpRequest::initCookies() {
    ...
    PARSE_PARAM(cookie, m_cookies, ';', jujimeizuo::StringUtil::Trim);
    m_parserParamFlag |= 0x4;
}
...
void HttpResponse::setCookie(const std::string &key, const std::string &val,
    ...
    ss << key << "=" << val;
    if (expired > 0) {
        ss << ";expires=" << jujimeizuo::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
    }
    if (!domain.empty()) {
        ss << ";domain=" << domain;
    }
