...
static jujimeizuo::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_mysql_dbs
    = jujimeizuo::Config::Lookup("mysql.dbs", std::map<std::string, std::map<std::string, std::string> >()
            , "mysql dbs");
...
static MYSQL* mysql_init(std::map<std::string, std::string>& params,
    ...
    if(mysql == nullptr) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "mysql_init error";
        return nullptr;
    }
    ...
    int port = jujimeizuo::GetParamValue(params, "port", 0);
    std::string host = jujimeizuo::GetParamValue<std::string>(params, "host");
    std::string user = jujimeizuo::GetParamValue<std::string>(params, "user");
    std::string passwd = jujimeizuo::GetParamValue<std::string>(params, "passwd");
    std::string dbname = jujimeizuo::GetParamValue<std::string>(params, "dbname");

    if(mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str()
                          ,dbname.c_str(), port, NULL, 0) == nullptr) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "mysql_real_connect(" << host
                                  << ", " << port << ", " << dbname
...
bool MySQL::connect() {
    ...
    m_hasError = false;
    m_poolSize = jujimeizuo::GetParamValue(m_params, "pool", 5);
    m_mysql.reset(m, mysql_close);
    return true;
}

jujimeizuo::IStmt::ptr MySQL::prepare(const std::string& sql) {
...
int MySQL::execute(const char* format, va_list ap) {
    m_cmd = jujimeizuo::StringUtil::Formatv(format, ap);
    int r = ::mysql_query(m_mysql.get(), m_cmd.c_str());
    if(r) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "cmd=" << cmd()
            << ", error: " << getErrStr();
        m_hasError = true;
    } else {
        m_hasError = false;
...
std::shared_ptr<MySQL> MySQL::getMySQL() {
    return MySQL::ptr(this, jujimeizuo::nop<MySQL>);
}
...
static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql) {
    if(mysql == nullptr) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "mysql_query mysql is null";
        return nullptr;
    }

    if(sql == nullptr) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "mysql_query sql is null";
        return nullptr;
    }

    if(::mysql_query(mysql, sql)) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:"
            << mysql_error(mysql);
        return nullptr;
    }
    ...
    if(res == nullptr) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "mysql_store_result() error:"
            << mysql_error(mysql);
    }
    return res;
}
...
MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string& stmt) {
    ...
    if(mysql_stmt_prepare(st, stmt.c_str(), stmt.size())) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "stmt=" << stmt
...
int MySQLStmt::bindTime(int idx, const time_t& value) {
    ...
    //return 0;
    return bindString(idx, jujimeizuo::Time2Str(value));
}
...
int64_t MySQLRes::getInt64(int idx) {
    return jujimeizuo::TypeUtil::Atoi(m_cur[idx]);
}
...
double MySQLRes::getDouble(int idx) {
    return jujimeizuo::TypeUtil::Atof(m_cur[idx]);
}
...
time_t MySQLRes::getTime(int idx) {
    ...
    return jujimeizuo::Str2Time(m_cur[idx]);
}
...
int MySQLTransaction::execute(const char* format, va_list ap) {
    if(m_isFinished) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "transaction is finished, format=" << format;
        return -1;
    }
...
int MySQLTransaction::execute(const std::string& sql) {
    if(m_isFinished) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
        return -1;
    }
    ...
    if(it != m_conns.end()) {
        if(!it->second.empty()) {
            ...
            if(rt->ping()) {
                ...
                JUJIMEIZUO_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                return nullptr;
            }
        }
    }
...
int MySQLManager::execute(const std::string& name, const char* format, va_list ap) {
    auto conn = get(name);
    if(!conn) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
            << ") fail, format=" << format;
        return -1;
    }
...
ISQLData::ptr MySQLManager::query(const std::string& name, const char* format, va_list ap) {
    auto conn = get(name);
    if(!conn) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
            << ") fail, format=" << format;
        return nullptr;
    }
...
MySQLTransaction::ptr MySQLManager::openTransaction(const std::string& name, bool auto_commit) {
    auto conn = get(name);
    if(!conn) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name
            << ") fail";
        return nullptr;
    }
