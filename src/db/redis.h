...
public:
    ...
    FoxRedis(jujimeizuo::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedis();
...
private:
    struct FCtx {
        std::string cmd;
        jujimeizuo::Scheduler* scheduler;
        jujimeizuo::Fiber::ptr fiber;
        ReplyPtr rpy;
    };
...
public:
    ...
    FoxRedisCluster(jujimeizuo::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
...
typedef jujimeizuo::Singleton<RedisManager> RedisMgr;
