#ifndef __SRC_WORKER_H__
#define __SRC_WORKER_H__
...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, jujimeizuo::Scheduler* s = jujimeizuo::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, jujimeizuo::Scheduler* s = jujimeizuo::Scheduler::GetThis());
    ~WorkerGroup();
...
typedef jujimeizuo::Singleton<WorkerManager> WorkerMgr;
