...
int Application::run_fiber() {
    ...
    for(auto& i : http_confs) {
        ...
        if(i.type == "http") {
            ...
            server.reset(new jujimeizuo::http::WSServer(
                            process_worker, io_worker, accept_worker));
        } else if(i.type == "rock") {
            server.reset(new jujimeizuo::RockServer("rock",
                            process_worker, io_worker, accept_worker));
        } else if(i.type == "nameserver") {
            server.reset(new jujimeizuo::RockServer("nameserver",
                            process_worker, io_worker, accept_worker));
            ModuleMgr::GetInstance()->add(std::make_shared<jujimeizuo::ns::NameServerModule>());
        } else {
        ...
        if(!server->bind(address, fails, i.ssl)) {
            for(auto& x : fails) {
                JUJIMEIZUO_LOG_ERROR(g_logger) << "bind address fail:"
                    << *x;
            }
            _exit(0);
        }
        if(i.ssl) {
            if(!server->loadCertificates(i.cert_file, i.key_file)) {
                JUJIMEIZUO_LOG_ERROR(g_logger) << "loadCertificates fail, cert_file="
    ...
    if(!g_service_discovery_zk->getValue().empty()) {
        ...
        if(!getServer("http", svrs)) {
            m_serviceDiscovery->setSelfInfo(jujimeizuo::GetIPv4() + ":0:" + jujimeizuo::GetHostName());
        } else {
            ...
            m_serviceDiscovery->setSelfInfo(ip_and_port + ":" + jujimeizuo::GetHostName());
        }
    }
    ...
    //jujimeizuo::ZKServiceDiscovery::ptr zksd(new jujimeizuo::ZKServiceDiscovery("127.0.0.1:21811"));
    //zksd->registerServer("blog", "chat", jujimeizuo::GetIPv4() + ":8090", "xxx");
    //zksd->queryServer("blog", "chat");
    //zksd->setSelfInfo(jujimeizuo::GetIPv4() + ":8090");
