...
bool Module::handleRequest(jujimeizuo::Message::ptr req
                           ,jujimeizuo::Message::ptr rsp
                           ,jujimeizuo::Stream::ptr stream) {
    JUJIMEIZUO_LOG_DEBUG(g_logger) << "handleRequest req=" << req->toString()
            << " rsp=" << rsp->toString() << " stream=" << stream;
    return true;
}

bool Module::handleNotify(jujimeizuo::Message::ptr notify
                          ,jujimeizuo::Stream::ptr stream) {
    JUJIMEIZUO_LOG_DEBUG(g_logger) << "handleNotify nty=" << notify->toString()
            << " stream=" << stream;
    return true;
}
...
bool Module::onConnect(jujimeizuo::Stream::ptr stream) {
    return true;
}

bool Module::onDisconnect(jujimeizuo::Stream::ptr stream) {
    return true;
}
    ...
    for(auto& i : svrs) {
        ...
        for(auto& s : socks) {
            ...
            if(str.find("0.0.0.0") == 0) {
                ip_and_port = jujimeizuo::GetIPv4() + ":" + std::to_string(addr->getPort());
            } else {
...
bool RockModule::handleRequest(jujimeizuo::Message::ptr req
                               ,jujimeizuo::Message::ptr rsp
                               ,jujimeizuo::Stream::ptr stream) {
    auto rock_req = std::dynamic_pointer_cast<jujimeizuo::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<jujimeizuo::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<jujimeizuo::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(jujimeizuo::Message::ptr notify
                              ,jujimeizuo::Stream::ptr stream) {
    auto rock_nty = std::dynamic_pointer_cast<jujimeizuo::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<jujimeizuo::RockStream>(stream);
...
void ModuleManager::init() {
    ...
    jujimeizuo::FSUtil::ListAllFile(files, path, ".so");
