...
bool NameServerModule::handleRockRequest(jujimeizuo::RockRequest::ptr request
                                         ,jujimeizuo::RockResponse::ptr response
                                         ,jujimeizuo::RockStream::ptr stream) {
    jujimeizuo::Atomic::addFetch(s_request_count, 1);
    switch(request->getCmd()) {
        ...
        default:
            JUJIMEIZUO_LOG_WARN(g_logger) << "invalid cmd=0x" << std::hex << request->getCmd();
            break;
    }
    return true;
}

bool NameServerModule::handleRockNotify(jujimeizuo::RockNotify::ptr notify
                                        ,jujimeizuo::RockStream::ptr stream) {
    return true;
}

bool NameServerModule::onConnect(jujimeizuo::Stream::ptr stream) {
    jujimeizuo::Atomic::addFetch(s_on_connect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    }
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        JUJIMEIZUO_LOG_INFO(g_logger) << "onConnect: " << *addr;
    }
    return true;
}

bool NameServerModule::onDisconnect(jujimeizuo::Stream::ptr stream) {
    jujimeizuo::Atomic::addFetch(s_on_disconnect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    ...
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        JUJIMEIZUO_LOG_INFO(g_logger) << "onDisconnect: " << *addr;
    }
...
NSClientInfo::ptr NameServerModule::get(jujimeizuo::RockStream::ptr rs) {
    jujimeizuo::RWMutex::ReadLock lock(m_mutex);
    auto it = m_sessions.find(rs);
    return it == m_sessions.end() ? nullptr : it->second;
}

bool NameServerModule::handleRegister(jujimeizuo::RockRequest::ptr request
                                      ,jujimeizuo::RockResponse::ptr response
                                      ,jujimeizuo::RockStream::ptr stream) {
    auto rr = request->getAsPB<RegisterRequest>();
    if(!rr) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "invalid register request from: "
...
void NameServerModule::set(jujimeizuo::RockStream::ptr rs, NSClientInfo::ptr new_value) {
    if(!rs->isConnected()) {
        new_value = nullptr;
    }
    ...
    //jujimeizuo::WorkerMgr::GetInstance()->schedule("notify",
...
std::set<jujimeizuo::RockStream::ptr> NameServerModule::getStreams(const std::string& domain) {
    jujimeizuo::RWMutex::ReadLock lock(m_mutex);
    auto it = m_domainToSessions.find(domain);
    return it == m_domainToSessions.end() ? std::set<jujimeizuo::RockStream::ptr>() : it->second;
}
...
bool NameServerModule::handleQuery(jujimeizuo::RockRequest::ptr request
                                   ,jujimeizuo::RockResponse::ptr response
                                   ,jujimeizuo::RockStream::ptr stream) {
    auto qreq = request->getAsPB<QueryRequest>();
    if(!qreq) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "invalid query request from: "
            << stream->getRemoteAddressString();
        return false;
    }
...
void NameServerModule::setQueryDomain(jujimeizuo::RockStream::ptr rs, const std::set<std::string>& ds) {
...
bool NameServerModule::handleTick(jujimeizuo::RockRequest::ptr request
                                  ,jujimeizuo::RockResponse::ptr response
                                  ,jujimeizuo::RockStream::ptr stream) {
    return true;
}
