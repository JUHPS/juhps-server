...
static jujimeizuo::ConfigVar<std::unordered_map<std::string
    ,std::unordered_map<std::string, std::string> > >::ptr g_rock_services =
    jujimeizuo::Config::Lookup("rock_services", std::unordered_map<std::string
...
RockStream::RockStream(Socket::ptr sock)
    ...
    JUJIMEIZUO_LOG_DEBUG(g_logger) << "RockStream::RockStream "
        << this << " "
        << (sock ? sock->toString() : "");
}
...
RockResult::ptr RockStream::request(RockRequest::ptr req, uint32_t timeout_ms) {
    if(isConnected()) {
        ...
        addCtx(ctx);
        uint64_t ts = jujimeizuo::GetCurrentMS();
        ctx->timer = jujimeizuo::IOManager::GetThis()->addTimer(timeout_ms,
                std::bind(&RockStream::onTimeOut, shared_from_this(), ctx));
        enqueue(ctx);
        jujimeizuo::Fiber::YieldToHold();
        return std::make_shared<RockResult>(ctx->result, jujimeizuo::GetCurrentMS() - ts, ctx->response, req);
    } else {
...
AsyncSocketStream::Ctx::ptr RockStream::doRecv() {
    //JUJIMEIZUO_LOG_INFO(g_logger) << "doRecv " << this;
    ...
    if(type == Message::RESPONSE) {
        ...
        if(!rsp) {
            JUJIMEIZUO_LOG_WARN(g_logger) << "RockStream doRecv response not RockResponse: "
                << msg->toString();
            return nullptr;
        }
        RockCtx::ptr ctx = getAndDelCtxAs<RockCtx>(rsp->getSn());
        if(!ctx) {
            JUJIMEIZUO_LOG_WARN(g_logger) << "RockStream request timeout reponse="
                << rsp->toString();
            return nullptr;
        }
        ...
        if(!req) {
            JUJIMEIZUO_LOG_WARN(g_logger) << "RockStream doRecv request not RockRequest: "
                << msg->toString();
            return nullptr;
        }
        if(m_requestHandler) {
            ...
            JUJIMEIZUO_LOG_WARN(g_logger) << "unhandle request " << req->toString();
        }
    } else if(type == Message::NOTIFY) {
        auto nty = std::dynamic_pointer_cast<RockNotify>(msg);
        if(!nty) {
            JUJIMEIZUO_LOG_WARN(g_logger) << "RockStream doRecv notify not RockNotify: "
                << msg->toString();
            return nullptr;
        }
        ...
        if(m_notifyHandler) {
            ...
            JUJIMEIZUO_LOG_WARN(g_logger) << "unhandle notify " << nty->toString();
        }
    } else {
        JUJIMEIZUO_LOG_WARN(g_logger) << "RockStream recv unknow type=" << type
            << " msg: " << msg->toString();
    }
    return nullptr;
}

void RockStream::handleRequest(jujimeizuo::RockRequest::ptr req) {
    jujimeizuo::RockResponse::ptr rsp = req->createResponse();
...
void RockStream::handleNotify(jujimeizuo::RockNotify::ptr nty) {
...
bool RockConnection::connect(jujimeizuo::Address::ptr addr) {
    m_socket = jujimeizuo::Socket::CreateTCP(addr);
...
static SocketStream::ptr create_rock_stream(ServiceItemInfo::ptr info) {
    jujimeizuo::IPAddress::ptr addr = jujimeizuo::Address::LookupAnyIPAddress(info->getIp());
    if(!addr) {
        JUJIMEIZUO_LOG_ERROR(g_logger) << "invalid service info: " << info->toString();
        return nullptr;
    }
    ...
    jujimeizuo::WorkerMgr::GetInstance()->schedule("service_io", [conn, addr](){
        conn->connect(addr);
        conn->start();
    });
    return conn;
...
RockResult::ptr RockSDLoadBalance::request(const std::string& domain, const std::string& service,
    ...
    uint64_t ts2 = jujimeizuo::GetCurrentMS();
    if(r->result == 0) {
        stats.incOks(1);
